...
1.当用户线程调用 select 的时候，select 将需要监控的 readfds集合 拷贝到内核空间（假设监控的仅仅是 socket可读）。
...
13. SO_REUSEADDR作用:
    快速重用端口：
    1. 在正常关闭服务器程序时，套接字的端口会进入 TIME_WAIT 状态。此时即使该端口处于 TIME_WAIT 状态，也可以允许新的程序（甚至是同一程序）重新绑定到该端口。
    
    2. 在一个多进程或多线程的服务器应用程序中，多个进程或线程可以绑定到同一个地址和端口，前提是它们都设置了 SO_REUSEADDR。这常用于 负载均衡 或 端口共享。